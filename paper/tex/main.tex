...
The evolution of mobile communication has led to an environment where diverse access technologies (e.g., CDMA, WLAN, WiMax, LTE) overlap. The core problem in this heterogeneous landscape is the Vertical Handover (VHO) decision: selecting the "best" network at the "best" time \cite{pahlavan2002principles}.
...
\begin{itemize}
\item \textbf{AUHO (Autonomic HandOver)}: The flagship algorithm based on Dr. Kang's PhD research \cite{kang2011autonomic}. It utilizes a two-stage APAV/APSV framework \cite{kang2011ieee_cns} to provide personalized, context-aware decisions.
...
This scenario recreates the urban multi-tech environment published in the Computer Networks (Elsevier) journal \cite{kang2011comnet}. It validates the capability of the AUHO algorithm to coordinate between macro-cells and hotspots under high-mobility conditions.
...
\section{Conclusion}
HMNToolSuite-ng represents a significant step forward in handover research tooling, building upon established wireless standards \cite{ieee2012standard}. By lowering the barrier to entry for implementing custom decision logic and providing a rich, interactive visualization suite, the platform enables researchers to focus on algorithmic innovation rather than infrastructure development.
